...
const toggleBGM = (value) => {
  if (value) AudioManager.playBGM('chat')
  else AudioManager.stopBGM(false)
  saveSettings()
}
...
const toggleDarkMode = (value) => {
  ...
  localStorage.setItem('night-city-theme', value ? 'dark' : 'light')
...
const checkUpdate = () => { showToast('已是最新版本') }
const showPrivacy = () => { showToast('隐私政策即将上线') }
const showTerms = () => { showToast('用户协议即将上线') }
